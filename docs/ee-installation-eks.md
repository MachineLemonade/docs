...
Note: the current AMI provided by AWS has a bug in it that prevents ElasticSearch from working correctly. In the link above `Step 3: Launch and Configure Amazon EKS Worker Nodes`, you will need to use the CloudFormation yaml provided below instead of the one provided in the AWS doc.
